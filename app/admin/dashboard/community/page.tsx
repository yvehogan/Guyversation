...
import { GetCommunitiesQuery, Community, PrivacyEnums, GetCommunitiesResponse } from "@/components/queries/communities/get-communities";
...
export default function CommunityPage() {
  ...
  useEffect(() => {
    const checkForSearchUpdates = () => {
      const savedSearch = localStorage.getItem("search-communities") || "";
      if (savedSearch !== search) {
        ...
        searchTimeoutRef.current = setTimeout(() => {
          queryClient.invalidateQueries({ queryKey: ["admin-communities", sortBy, currentPage, pageSize] });
        }, 300);
      }
    };
  ...
  }, [search, sortBy, currentPage, pageSize, queryClient]);

  const { data, isLoading, isError } = useQuery<GetCommunitiesResponse>({
    queryKey: ["admin-communities", sortBy, currentPage, pageSize, search],
    queryFn: async () => {
      try {
        const result = await GetCommunitiesQuery({
          page: currentPage,
          pageSize: pageSize,
