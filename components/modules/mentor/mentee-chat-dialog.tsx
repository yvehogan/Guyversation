...
import {
  GetMessagesQuery,
  type MessageData,
} from "@/components/queries/chat/get-messages";
...
export function MenteeChatDialog({
  mentee,
  open,
  onOpenChange,
}: MenteeChatDialogProps) {
      ...
      try {
        ...
        if (response.isSuccess && response.data) {
          const formattedMessages: Message[] = response.data.map(
            (msgData: MessageData) => ({
              id: msgData.id,
              sender: msgData.userId === mentee.id ? "mentee" : "mentor",
  ...
  const initializeChat = async () => {
    setIsLoading(true);
    try {
      const chatResponse = await StartChatQuery({ peerUserId: mentee.id });
  ...
  const generateEmojis = () => {
    const emojiCategories = {
      ...
      hearts: [
        0x2764, 0x1f49b, 0x1f49a, 0x1f499, 0x1f49c, 0x1f5a4, 0x1f90d, 0x1f90e,
      ],
      symbols: [
        0x2728, 0x1f525, 0x1f4af, 0x1f389, 0x1f38a, 0x1f44f, 0x1f44d, 0x1f44e,
      ],
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="right-0 mt-3 mr-3 md:mr-5 h-auto max-h-[97vh] w-[95%] overflow-hidden rounded-lg border-0 py-4 sm:max-w-md flex flex-col">
